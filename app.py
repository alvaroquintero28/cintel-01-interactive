...
ui.page_opts(title="To Be or Not To Be", fillable=True)
...
def histogram():
    ...
    # Create a histogram plot using the selected number of bins and red color
    plt.hist(x, input.selected_number_of_bins(), density=True, color='red')

@render.plot(alt="A histogram")
def scatterplot():
    np.random.seed(34)
    x = 100 + 15 * np.random.normal(size=10)
    y = 1 + 1 * np.random.normal(size=10)
